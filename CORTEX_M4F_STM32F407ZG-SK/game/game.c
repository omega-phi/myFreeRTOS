...
uint8_t demoMode = 0;
uint8_t acceMode = 0;
uint16_t acceModeCount = 5;

//acceleration
int16_t acceX = 0;
int16_t acceY = 0;
int16_t fieldIntensity = 12;	//the smaller number the greater intensity
void field();
void changeField();
...
field()
{
	if(acceMode==0)		//noChange
	{
		acceX+=0;
		acceY+=0;
	}
	else if(acceMode==1)	//acceleration to wall
	{
		acceX+=(ballX<120)?-3:3;
		acceY+=0;
	}
	else if(acceMode==2)	//acceleration to both player
	{
		acceX+=0;
		acceY+=(ballY<160)?-1:1;
	}
	else if(acceMode==3)	//acceleration downward
	{
		acceX+=0;
		acceY+=1;
	}
	else if(acceMode==4)	//acceleration upward
	{
		acceX+=0;
		acceY+=-1;
	}

	if(acceX>=fieldIntensity)	//if acceX accumulate to wanted itensity => apply change
	{
		acceX=0;
		ballVX+=1;
	}
	else if(acceX<=fieldIntensity*(-1))
	{
		acceX=0;
		ballVX-=1;
	}

	if(acceY>=fieldIntensity)	//if acceY accumulate to wanted itensity => apply change
	{
		acceY=0;
		ballVY+=1;
	}
	else if(acceY<=fieldIntensity*(-1))
	{
		acceY=0;
		ballVY-=1;
	}
...
changeField()
{
	acceMode=(acceMode+1)%acceModeCount;	//change field mode
}
